...
utils.fs.copyFile = (srcPath, tgtPath, copy, callback) => {
	...
	*/
	...
	streams.write.tgt
		.on(constants.events.error, err => {
			...
			streamEmitter.emit(
				...
				Error('target-write error: ' + utils.fs.handleFileError(err, tgtPath)) )

		})
		.on(constants.events.finish, ( ) => {
			...
			streams.read.tgt  =
				fs.createReadStream(tgtPath)
					.on(constants.events.error, err => {
						...
						streamEmitter.emit(
							constants.events.error,
							Error('target-read error: ' + utils.fs.handleFileError(err, tgtPath)) )

					})
					.on(constants.events.end, ( ) => {

						hashes.tgt.end( )
						streamEmitter.emit(constants.events.targetChecksum, hashes.tgt.read( ))
	...
	streams.read.src
		.on(constants.events.error, err => {
			...
			streamEmitter.emit(
				constants.events.error,
				Error('source-read error: ' + utils.fs.handleFileError(err, srcPath)) )

		})
		.on(constants.events.end, ( ) => {

			hashes.src.end( )
			streamEmitter.emit(constants.events.sourceChecksum, hashes.src.read( ))
	...
	*/
	...
	streamEmitter
		...
		.on(constants.events.sourceChecksum, srcHash => {
			streamEmitter.on(constants.events.targetChecksum, tgtHash => {

				if (tgtHash !== srcHash) {
					return singleCallback(Error('checksum mismatch between ${srcPath} and ${tgtHash}; reverting.\n'))
				}

				utils.fs.copyUtimes(srcPath, tgtPath, err => {
					...
					if (!copy) {
						...
						fs.unlink(srcPath, err => {
							...
							if (err) {
								return singleCallback(Error('error while attempting to remove ${srcPath}: ${err.message}\n'))
							}
...
utils.fs.copyUtimes = (srcPath, tgtPath, callback) => {
	...
	fs.lstat(srcPath, (err, stats) => {
		...
		fs.utimes(tgtPath, stats.atime, stats.mtime, err => {
