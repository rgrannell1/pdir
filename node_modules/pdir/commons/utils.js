...
utils.fs.moveFile = (srcPath, tgtPath, callback) => {
	...
	const sourceStream = fs.createReadStream(srcPath)
	const targetStream = fs.createWriteStream(tgtPath)

	utils.crypto.checksum(sourceStream, (err, readChecksum) => {

	})

	sourceStream.on(constants.events.error, err => {
		done(err)
	})

	targetStream.on(constants.events.error, err => {
		done(err)
	})

	targetStream.on(constants.events.close, ( ) => {
		// get target checksum
		done(null)
	})
